...
describe('AppCardNav', () => {
  it('renders card menu entries without top AI CTA', async () => {
    ...
    expect(screen.queryByText('题材分析')).not.toBeInTheDocument()
    ...
    expect(dashboardCard).toContainElement(screen.getByRole('link', { name: '进入题材库' }))
    expect(screen.getByRole('link', { name: '打开 AI 个股分析' })).toHaveAttribute(
      'href',
      '/ai-analysis'
    )
    expect(reviewCard).toContainElement(screen.getByRole('link', { name: '打开 AI 个股分析' }))
